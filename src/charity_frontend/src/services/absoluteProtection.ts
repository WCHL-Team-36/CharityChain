...
export const absoluteProtection = async <T>(operation: () => Promise<T>, operationName: string): Promise<{ success: boolean; data?: T; error?: string }> => {
  try {
...
export const approveTransactionAbsolute = async (
  amount: number
): Promise<{
  success: boolean;
  error?: string;
