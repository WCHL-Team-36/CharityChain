    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100 py-12 px-4">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="text-center mb-12">
          ...
          <p className="text-xl text-gray-600 mb-8">Discover and support amazing causes in our community</p>
          <button onClick={loadCampaigns} className="bg-indigo-600 hover:bg-indigo-700 text-white font-bold py-2 px-6 rounded-lg transition duration-200">
            🔄 Refresh Campaigns
          </button>
        </div>
        ...
        {campaigns.length === 0 ? (
          <div className="text-center py-16">
            <div className="bg-white rounded-lg shadow-xl p-12 max-w-2xl mx-auto">
              ...
              <p className="text-gray-600 mb-8">Be the first to create a campaign and make a difference!</p>
              <a href="/create-campaign" className="bg-indigo-600 hover:bg-indigo-700 text-white font-bold py-3 px-8 rounded-lg transition duration-200 inline-block">
              ...
              <div key={campaign.id} className="bg-white rounded-lg shadow-xl overflow-hidden hover:shadow-2xl transition-shadow duration-300">
                ...
                <div className="bg-gradient-to-r from-indigo-500 to-purple-600 p-6">
                  ...
                  <div className="flex items-center mt-2">
                    <span className={`px-3 py-1 rounded-full text-sm font-medium ${campaign.isActive ? "bg-green-100 text-green-800" : "bg-red-100 text-red-800"}`}>{campaign.isActive ? "🟢 Active" : "🔴 Inactive"}</span>
                  </div>
                </div>
                ...
                <div className="p-6">
                  ...
                  <div className="mb-4">
                    ...
                    <div className="w-full bg-gray-200 rounded-full h-3">
                      <div className="bg-gradient-to-r from-green-400 to-blue-500 h-3 rounded-full transition-all duration-500" style={{ width: `${calculateProgress(campaign.currentAmount, campaign.goalAmount)}%` }}></div>
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-4 mb-4">
                    <div className="text-center">
                      ...
                      <p className="text-lg font-bold text-green-600">${formatAmount(campaign.currentAmount)}</p>
                    </div>
                    <div className="text-center">
                      <p className="text-sm text-gray-500">Target</p>
                      <p className="text-lg font-bold text-indigo-600">${formatAmount(campaign.goalAmount)}</p>
                    </div>
                  </div>
        ...
        {campaigns.length > 0 && (
          <div className="mt-16 bg-white rounded-lg shadow-xl p-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
              ...
              <div className="text-center">
                <div className="text-3xl font-bold text-green-600">{campaigns.filter((c) => c.isActive).length}</div>
                <div className="text-gray-600">Active Campaigns</div>
              </div>
              <div className="text-center">
                <div className="text-3xl font-bold text-blue-600">${campaigns.reduce((sum, c) => sum + Number(formatAmount(c.goalAmount)), 0).toFixed(2)}</div>
                <div className="text-gray-600">Total Target</div>
              </div>
              <div className="text-center">
                <div className="text-3xl font-bold text-purple-600">${campaigns.reduce((sum, c) => sum + Number(formatAmount(c.currentAmount)), 0).toFixed(2)}</div>
